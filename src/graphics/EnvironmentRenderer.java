...
public class EnvironmentRenderer {
    ...
    private void drawTilemap(Graphics g) {
        TileMap tileMap = panel.getEnvironment().getTileMap();
    ...
    private void drawPause(Graphics g) {
        if (panel.getEnvironment().getPaused()) {
            // TODO
            g.setColor(Color.BLACK);
            g.setFont(new Font("TimesNewRoman", Font.PLAIN, 40));
            g.drawString("PAUSED", panel.getWidth() / 2 - 100, panel.getHeight() * 3 / 4);
        }
    }
    ...
    public void drawAgent(Graphics g, Agent agent, int x, int y, float scale) {
        ...
        g.setColor(new Color(DNA[0] - Byte.MIN_VALUE, DNA[1] - Byte.MIN_VALUE, 0));
